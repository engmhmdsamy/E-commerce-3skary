...
import { UserContext } from "../TokenContext/TokenContext"; // استرجع UserContext
import toast from "react-hot-toast";

export default function Products() {
  let { addToCart, addToWishList, wishList, setwishList, GetWishList } = useContext(cartContext);
  const { cartCount, setCartCount } = useContext(UserContext); // استرجع cartCount و setCartCount
  ...
  async function addProToCart(id) {
    ...
    if (data?.status === "success") {
      ...
      setCartCount(cartCount + 1); // زيادة عدد المنتجات في العربة
    } else {
  ...
  return (
    <>
      <Helmet>
        <title>{window.location.pathname.split("/").pop()}</title>
      </Helmet>
      {isLoading ? (
        ...
        <div className="row">
          ...
          {data?.data.data.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="col-md-6 col-lg-4 col-xl-2 product p-2 mb-3 rounded-3"
              >
                ...
                <button
                  style={{ color: wishListIds?.includes(product.id) ? "red" : "black" }}
